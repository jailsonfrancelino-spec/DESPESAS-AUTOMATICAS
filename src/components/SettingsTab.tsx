...
import { Expense, ExpenseEntity, NotificationSettings, NavigationMode } from '../types';
import {
  exportExpensesToCSV,
  formatCurrency,
  INITIAL_EXPENSES,
...
export const SettingsTab: React.FC<SettingsTabProps> = ({
  ...
  return (
    <div className="space-y-5 px-3 sm:px-6 pb-20 pt-1">
      ...
      {/* Modo de Visualização & Navegação: iPhone vs MacBook */}
      <section className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/90 shadow-xs space-y-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">
                Modo de Visualização &amp; Navegação
              </h3>
              <p className="text-xs text-slate-500">
                Alterne entre visualização de Celular (iPhone) ou Computador (MacBook)
              </p>
            </div>
          </div>

          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 uppercase tracking-wider">
            {navMode === 'macbook' ? 'Modo Mac Ativo' : 'Modo iPhone Ativo'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
          ...
          <button
            type="button"
            onClick={() => onToggleNavMode && onToggleNavMode('iphone')}
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                <Smartphone className={`w-4 h-4 ${navMode === 'iphone' ? 'text-blue-600' : 'text-slate-500'}`} />
                <span className="font-bold text-xs text-slate-900">📱 Celular iPhone</span>
              </div>
              {navMode === 'iphone' && (
            ...
            <p className="text-[11px] text-slate-600 leading-relaxed">
              Design vertical compacto otimizado para navegação por toque e tela do iPhone.
            </p>
          </button>
          ...
          <button
            type="button"
            onClick={() => onToggleNavMode && onToggleNavMode('macbook')}
            className={`p-3.5 rounded-xl border text-left transition-all ${
              navMode === 'macbook'
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                <Laptop className={`w-4 h-4 ${navMode === 'macbook' ? 'text-amber-600' : 'text-slate-500'}`} />
                <span className="font-bold text-xs text-slate-900">💻 Computador MacBook</span>
              </div>
              {navMode === 'macbook' && (
            ...
            <p className="text-[11px] text-slate-600 leading-relaxed">
              Layout expandido com 3 colunas simultâneas (Pessoal, Academia, Bets), barra macOS e atalhos.
            </p>
          </button>
        </div>
      </section>
      ...
      <section className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/90 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Sessão salva automaticamente no dispositivo via Supabase e LocalStorage
              </p>
            </div>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-0.5">
            <span className="text-xs font-semibold text-slate-500 block">Usuário Conectado:</span>
            <div className="flex items-center gap-2">
              <span className="font-mono text-sm font-bold text-slate-900">
                {authUser?.username || 'jailson12'}
              </span>
            ...
            <p className="text-[11px] text-slate-500">
              {authUser?.email || 'jailson12@hotmail.com'}
            </p>
          </div>
          ...
          {onLogout && (
            <button
              type="button"
              onClick={onLogout}
              className="px-3 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 text-xs font-bold transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer"
            >
        ...
        <div className="text-xs text-slate-600 bg-emerald-50/50 p-3 rounded-xl border border-emerald-200/60 space-y-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            {onOpenSupabaseDiagnostic && (
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 active:scale-95 text-white font-bold text-xs shadow-xs transition-all flex items-center gap-1.5 cursor-pointer w-fit"
              >
                <Database className="w-3.5 h-3.5 text-emerald-200" />
                <span>Verificar Tabelas e Conexão</span>
              </button>
            )}
          </div>
          <p className="text-[11px] text-slate-600 leading-relaxed">
            O arquivo <code className="font-mono bg-white px-1.5 py-0.5 rounded border border-slate-300 text-slate-800">src/supabaseClient.ts</code> está integrado com <code className="font-mono text-emerald-800">@supabase/supabase-js</code>. Clique no botão acima para testar e validar o status de cada tabela (<code className="font-mono text-[10px]">expenses</code>, <code className="font-mono text-[10px]">categories</code>, <code className="font-mono text-[10px]">notification_settings</code>).
          </p>
        </div>
      </section>
