...
export const NewExpenseTab: React.FC<NewExpenseTabProps> = ({
  ...
  // When user changes entity, ensure the category remains valid
      ...
      >
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            >
              Modalidade / Categoria <span className="text-rose-500">*</span>
            </label>
            {onOpenCategoriesModal && (
              <button
                type="button"
                ...
                title="Criar ou editar modalidades e símbolos"
              >
                <Layers className="w-3 h-3" />
                <span>+ Criar / Editar Modalidades</span>
              </button>
            )}
          </div>
          ...
          <select
            ...
            className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-900 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
          >
            {categories && categories.length > 0
              ? categories.map(cat => (
                  <option key={cat.id} value={cat.nome}>
                    {cat.simbolo ? `${cat.simbolo} ` : ''}
                    {cat.nome}
                  </option>
                ))
          ...
          {categories && categories.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {categories.slice(0, 9).map(c => {
                const isSelected = categoria === c.nome;
                return (
                  <button
                    key={c.id}
                    type="button"
